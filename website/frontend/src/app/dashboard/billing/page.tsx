...
function getStatusConfig(status: string) {
  switch (status.toLowerCase()) {
    ...
    default:
      return { icon: CreditCard, color: "text-gray-400", bg: "bg-gray-500/10", label: status };
  }
}
    ...
    <DashboardLayout>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          >
            <h2 className="text-sm font-semibold text-white mb-4">Current Subscription</h2>

            <div className="space-y-4">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${statusConfig?.bg || "bg-blue-500/10"}`}>
                    {statusConfig ? <statusConfig.icon className={`w-5 h-5 ${statusConfig.color}`} /> : <Clock className="w-5 h-5 text-blue-400" />}
                  </div>
                  <div>
                    <p className="font-medium text-white">{subscription?.plan_name || "Free Trial"}</p>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${statusConfig?.bg || "bg-blue-500/10"} ${statusConfig?.color || "text-blue-400"}`}>
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-center gap-3">
                        <div className="text-center">
                          <div className="bg-blue-500/20 rounded-lg px-4 py-2 min-w-15">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-white/5">
                  ...
                  {daysRemaining !== null && subscription.status?.toLowerCase() !== "trialing" && (
                    <div>
                      ...
                      <p className={`text-sm font-medium ${daysRemaining <= 3 ? "text-red-400" : "text-white"}`}>
              ...
              {subscription?.canceled_at && (
                <div className="pt-3 border-t border-white/5">
                  <p className="text-xs text-gray-500">Canceled on {formatDate(subscription.canceled_at)}</p>
                </div>
              )}
            </div>
          </motion.div>
                    ...
                    >
                      ...
                      <ul className="space-y-2 grow">
                        {plan.features.map((feature) => (
                          <li key={feature} className={`flex items-center gap-2 text-sm ${isLimitedFeature(feature) ? "text-gray-500" : "text-gray-300"}`}>
                      ...
                      <div className="mt-4">
                        {plan.isFree ? (
                          isCurrentPlan ? (
                            ...
                            <Button
                              className="w-full shadow-none bg-white/10 text-gray-400 border border-white/10 cursor-default"
                              size="sm"
                              disabled
                            >
                          ...
                          <Button
                            className="w-full shadow-none bg-green-500/20 text-green-400 border border-green-500/30 cursor-default"
                            size="sm"
                            disabled
                          >
                            <CheckCircle className="w-3.5 h-3.5" />
          ...
          >
            <h2 className="text-sm font-semibold text-white mb-4">Billing History</h2>
            ...
            <div className="space-y-2">
              ...
              {(!subscription || subscription.status?.toLowerCase() === "trialing") && (
                <div className="flex items-center justify-between py-3 border-b border-white/5">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-white">$0.00 USD</p>
              ...
              {transactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-white">{tx.description || "Subscription payment"}</p>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {subscription && subscription.status?.toLowerCase() !== "canceled" && (
                <div className="flex items-center justify-between p-4 bg-surface-elevated rounded-lg border border-white/5">
                  <div>
              ...
              <div className="flex items-center justify-between p-4 bg-red-500/5 rounded-lg border border-red-500/20">
                <div>
                  <p className="text-sm font-medium text-white">Close Account</p>
                  <p className="text-xs text-gray-500 mt-1">
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <motion.div
            className="bg-surface-card rounded-xl border border-white/10 p-6 max-w-md w-full"
